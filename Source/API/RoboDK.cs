...
public class RoboDK : IRoboDK, IDisposable
{
    ...
    internal void send_pose(Mat pose)
    {
        ...
        {
            throw new($"Matrix not Homogenous: {pose.Cols}x{pose.Rows}");
        }
    ...
    internal void MoveX(IItem target,
        double[] joints,
        Mat matTarget,
        IItem itemrobot,
        int movetype,
    ...
    // private move type, to be used by public methods (MoveJ  and MoveL)
    internal void moveC_private(IItem target1,
        double[] joints1,
        Mat matTarget1,
        IItem target2,
        double[] joints2,
    ...
    public IRoboDK CloneRoboDkConnection(ConnectionType connectionType = ConnectionType.Api)
    {
        var rdk = new RoboDK
        {
            RoboDKServerStartPort = RoboDKServerStartPort,
            RoboDKServerEndPort = RoboDKServerEndPort,
            Name = Name,
            Process = Process,
            ItemInterceptFunction = ItemInterceptFunction,
    ...
    public IntPtr GetWindowHandle()
    {
        ...
        return new(Convert.ToInt32(mainWindowId));
    }
    ...
    /// <inheritdoc />
    public IItem AddCurve(Mat curvePoints,
        IItem referenceObject = null,
        bool addToRef = false,
    ...
    /// <inheritdoc />
    public Mat ProjectPoints(Mat points,
        IItem objectProject,
        ProjectionType projectionType = ProjectionType.AlongNormalRecalc)
    {
        check_connection();
    ...
    /// <inheritdoc />
    public void StereoCameraMeasure(out Mat pose1,
        out Mat pose2,
        out int npoints1,
        out int npoints2,
        out int time,
    ...
    /// <inheritdoc />
    public double[] CalibrateTool(Mat posesJoints,
        out double[] errorStats,
    ...
    /// <inheritdoc />
    public int ProgramStart(string progname,
    ...
    public IItem GetCursorXYZ(int xCoord = -1, int yCoord = -1, List<double> xyzStation = null)
    {
        ...
        if (xyz != null)
        {
            xyzStation.Add(xyz[0]);
            xyzStation.Add(xyz[1]);
            xyzStation.Add(xyz[2]);
        }
    ...
    public IItem AddTargetJ(IItem pgm, string targetName, double[] joints, IItem robotBase = null, IItem robot = null)
    {
        ...
        if (target == null)
        {
            throw new($"Create target '{targetName}' failed.");
        }
