...
internal class Item : IItem
{
    ...
    {
        var item = new Item((RoboDK)connectionLink, ItemId, _type);
    ...
    public bool ConnectSafe(string robotIp = "", int maxAttempts = 5, int waitConnection = 4)
    {
        ...
        while (true)
        {
            ...
            if (connectionStatus == RobotConnectionType.Disconnected)
            {
    ...
    public RobotConnectionParameter ConnectionParams()
    {
        ...
        Link.check_status();
        return new(robotIp, port, remotePath, ftpUser, ftpPass);
    }
    ...
    public void SetSpeed(double speedLinear,
        double accelLinear = -1,
        double speedJoints = -1,
    ...
    /// <inheritdoc />
    public void AddCustomInstruction(string name,
        string pathRun,
        string pathIcon = "",
        bool blocking = true,
