...
public class Mat
{
    ...
    public Mat(double nx,
        ...
        double oz,
        double az,
        double tz) // Matrix Class constructor
    {
        Rows = 4;
        Cols = 4;
        _mat = new double[Rows, Cols];
    ...
    public static Mat rotx(double rx)
    {
        var cx = Math.Cos(rx);
        var sx = Math.Sin(rx);
        return new(1, 0, 0, 0, 0, cx, -sx, 0, 0, sx, cx, 0);
    }
    ...
    public static Mat roty(double ry)
    {
        var cy = Math.Cos(ry);
        var sy = Math.Sin(ry);
        return new(cy, 0, sy, 0, 0, 1, 0, 0, -sy, 0, cy, 0);
    }
    ...
    public static Mat rotz(double rz)
    {
        var cz = Math.Cos(rz);
        var sz = Math.Sin(rz);
        return new(cz, -sz, 0, 0, sz, cz, 0, 0, 0, 0, 1, 0);
    }
    ...
    public Mat Rot3x3()
    {
        ...
        return new(_mat[0, 0], _mat[0, 1], _mat[0, 2],
    ...
    public static Mat FromXYZRPW(double x, double y, double z, double w, double p, double r)
    {
        ...
        var cc = Math.Cos(c);
        var sc = Math.Sin(c);
        return new(cb * cc, cc * sa * sb - ca * sc, sa * sc + ca * cc * sb, x, cb * sc, ca * cc + sa * sb * sc,
    ...
    public static Mat FromTxyzRxyz(double x, double y, double z, double rx, double ry, double rz)
    {
        ...
        var crz = Math.Cos(c);
        var srz = Math.Sin(c);
        return new(cry * crz, -cry * srz, sry, x, crx * srz + crz * srx * sry, crx * crz - srx * sry * srz,
    ...
    public static Mat FromUR(double[] xyzwpr)
    {
        ...
        var uy = p / angle;
        var uz = r / angle;
        return new(ux * ux + c * (1 - ux * ux), ux * uy * (1 - c) - uz * s, ux * uz * (1 - c) + uy * s, x,
    ...
    */
    ...
    public string ToString(bool string_as_xyzabc = true) // Function returns matrix as a string
    {
        ...
        if (string_as_xyzabc)
        {
            ...
            var values = ToTxyzRxyz();
            for (int j = 0; j < 6; j++)
            {
                s += letter[j] + string.Format("{0,6:0.000}", values[j]) + " " + units[j] + " , ";
            }
    ...
    public static double[] normalize3(double[] p)
    {
        ...
        return new[] { p[0] * norminv, p[1] * norminv, p[2] * norminv };
    }
    ...
    public static Mat xyzijk_2_pose(double[] point, double[] zaxis, double[] yaxis_hint = null)
    {
        ...
        pose.setPos(point);
        pose.setVZ(zaxis);
        if (angle3(zaxis, yaxis_hint) < 2 * Math.PI / 180)
        {
            yaxis_hint = new double[] { 0, 1, 1 };
        }

        double[] xaxis = normalize3(cross(yaxis_hint, zaxis));
...
public static Mat Parse(string ps)                        // Function parses the matrix from string
{
    ...
    private static double[] ToQuaternion(Mat Ti)
    {
        ...
        else if (cosangle < -1.0 + Tolerance_180)
        {
            ...
            double[] diag = { Ti[0, 0], Ti[1, 1], Ti[2, 2] };
            int k = Array.IndexOf(diag, diag.Max());
            double[] col = { Ti[0, k], Ti[1, k], Ti[2, k] };
            col[k] = col[k] + 1.0;
